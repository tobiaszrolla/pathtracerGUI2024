...
__host__ __device__ Camera::Camera(float fov, float aspectRatio,Vector3D position,float how_farX, float how_farY, float how_farZ)
{
    //origin = Vector3D(position.getX() + how_farX ,position.getY() + how_farY ,position.getZ() + how_farZ);
    //target = position;
    origin = Vector3D(0,-10,0);
    target = Vector3D(0,10,0);//kierunek patrzenia
...
__host__ __device__ Vector3D Camera::getOrigin() const{
    return origin;
}